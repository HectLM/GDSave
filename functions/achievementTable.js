let achievementCategories = [
    ...
    { id: "insane", stat: "insaneLevels", name: "Insane Levels", icon: "insane", color: "#FF64DC" },
    ...
    { id: "destroyPlayers", stat: "destroyedPlayers", name: "Destroyed Menu Icons", icon: "death", color: "#FF7D7D" }
]

let rewardTypes = { icon: "cube", bird: "ufo", dart: "wave" }
let specialRewardTypes = { special: "trail", death: "deathEffect" }

async function buildAchList() {

    let achTiers = await fetch("resources/acheivementTiers.json?v=1b").then(res => res.json())
