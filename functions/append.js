...
function labelObject(data={}, keys, keyName) {
    ...
    Object.keys(data).forEach(x => {
        if (x == "kCEK") return //newObj.encoder = kcekKeys[data[x]]
            ...
            else if (keyStuff.b64 && data[x].length >= 4) {
                try { newObj[keyName] = urlB64(data[x]) }
                catch(e) { newObj[keyName] = data[x] }
            }
...
function rewardObject(reward, type) {
    ...
    if (type == "treasureRoom") {
        if (rewardObj.chest.endsWith("key")) rewardObj.name = rewardObj.chest.split("key")[0] + " key"
        else if (rewardObj.chest == "gold") rewardObj.name = "Gold"
    }
