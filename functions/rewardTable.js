...
function appendRewards(rewardList) {
    ...
    rewardList.forEach((x, n) => {
        ...
        x.rewards.forEach(i => {
            if (i.item.endsWith(" Shard")) {
            ...
            else if (i.item == "Gold Key") {
                return iconStr += `<p><img style="height: 40px" src="assets/goldKey.png">Gold Key${i.amount > 1 ? ` (${i.amount})` : ""}</p>`   
            }
                ...
                else iconImg.attr("src", `https://gdbrowser.com/iconkit/premade/${i.iconForm}_${i.iconID}.png`)
        ...
        let chestID = (x.id || "")
        if (x.name == "Wraith") chestID = chestID.split("_")[2]
...
function flipChestTypes() {
    $('.chestType').each(function() {
        $(this).attr('enabled', $(this).attr('enabled') == "true" ? "false" : "true")
    })
    syncAllowedChests()
    appendRewards()
}