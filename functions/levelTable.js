function countRewards(lvlData) {
    ...
    let hasDiamonds = ["Daily", "Weekly", "Event", "Gauntlet"].includes(lvlData.type)
...
function appendLevels(lvlList, res=[]) {
        ...
        lvlList.forEach(x => {
           ...
           let isTimely = ["Daily", "Weekly", "Event"].includes(x.type)
           ...
           if (!x.difficulty) {
            if (x.estimatedDifficulty == "Demon" && x.demonDifficulty) x.difficulty = `${x.demonDifficulty} Demon`
           }

           difficultyFace = `${!x.demon ? (x.difficulty || x.estimatedDifficulty || "Unrated").toLowerCase() : `demon-${(x.demonDifficulty || "hard").toLowerCase().split(' ')[0]}`}${x.epic ? `-${x.epic.toLowerCase()}` : `${x.featuredPosition ? '-featured' : ''}`}`
           ...
           let ratings = {
            ...
            if (isTimely) typeStr = `${x.type} #${x.timelyID - (x.type == "Event" ? 200000 : x.type == "Weekly" ? 100000 : 0)}`
            ...
            let lvName = x.name
            if (x.buggedID) lvName = `${officialLevelNames[x.buggedID] || "Unknown"}?`
            ...
            <tr class="levelRow"><td>
                <p style="color: ${x.percentage == 100 ? "lime" : x.practicePercentage == 100 ? "cyan" : "white"};${x.buggedID ? " opacity: 66%" : ""}">${lvName || "(not saved)"}</p>
                <p ${x.author ? "" : 'style="display: none"'}>By ${official ? (x.author || "RobTop") : x.author || "[unknown]"}</p>
                ${official ? `<p>ID: ${x.buggedID || x.id || "N/A"}</p>` : `<p>ID: <a style="color: aqua !important" href="https://gdbrowser.com/${x.id}" target="_blank">${x.id}</a></p>`}
