...
async function kowalskiAnalysis(CCGameManager) {
    ...
    levelInfo.forEach((x, y) => {
        ...
        data[x].forEach(x => {
            ...
            if (x.type == "Official" && x._key != 1 && x.name == "Stereo Madness") x.buggedID = x._key
    ...
    addStatDiv("Currencies", [
        ...
        [`${commafy(data.stats.goldKeys)} / ${commafy(data.treasureRoomRewards.gold.length)}`, "Gold Keys", "goldKey"],
    ])
    ...
    levels.forEach(x => {
        if (!x.percentage || x.percentage < 100) return;
